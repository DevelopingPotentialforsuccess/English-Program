                 ...
                 <h3 className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Assessment Matrix</h3>
                  <div className="flex bg-[#111827] p-1.5 rounded-2xl border border-[#1f2937] gap-1 overflow-x-auto no-scrollbar">
                    ...
                    <button 
                      ...
                      className={`flex-1 min-w-[150px] py-4 px-6 rounded-xl transition-all ${(viewMode as string) === 'book_creation' ? 'bg-accent-blue text-white shadow-lg' : 'text-slate-600 hover:text-slate-400'}`}
                    >
                      <div className="text-[11px] font-black tracking-widest">Book Creation</div>
                    </button>
                    <button 
                      onClick={() => setViewMode('ielts_master')} 
                      className={`flex-1 min-w-[150px] py-4 px-6 rounded-xl transition-all ${(viewMode as string) === 'ielts_master' ? 'bg-accent-blue text-white shadow-lg' : 'text-slate-600 hover:text-slate-400'}`}
                    >
                      <div className="text-[11px] font-black tracking-widest">IELTS Master</div>
                    </button>
                    <button 
                      onClick={() => setViewMode('dpss_studio')} 
                      className={`flex-1 min-w-[150px] py-4 px-6 rounded-xl transition-all ${(viewMode as string) === 'dpss_studio' ? 'bg-accent-blue text-white shadow-lg' : 'text-slate-600 hover:text-slate-400'}`}
                    >
                      <div className="text-[11px] font-black tracking-widest">DPSS Studio</div>
                    </button>
                    <button 
                      onClick={() => setViewMode('khmer_program')} 
                      className={`flex-1 min-w-[150px] py-4 px-6 rounded-xl transition-all ${(viewMode as string) === 'khmer_program' ? 'bg-accent-blue text-white shadow-lg' : 'text-slate-600 hover:text-slate-400'}`}
                    >
                      <div className="text-[11px] font-black tracking-widest">Khmer Program</div>
                    </button>
                  </div>
              </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {[
                        ...
                        <div key={engine.id} className={`p-8 rounded-[40px] border-2 transition-all ${activeEngine === engine.id ? 'bg-white border-orange-600 shadow-xl' : 'bg-slate-50 border-slate-100'}`}>
                          ...
                          <div className="space-y-4">
                            {(engine.id === NeuralEngine.GEMINI_3_FLASH || engine.id === NeuralEngine.GEMINI_3_PRO) && (
                              <button 
                                onClick={async () => {
                            ...
                            <input 
                              ...
                              placeholder={ (engine.id === NeuralEngine.GEMINI_3_FLASH || engine.id === NeuralEngine.GEMINI_3_PRO) ? "Or Paste Custom Gemini Key" : "Custom API Key (Optional)" }
